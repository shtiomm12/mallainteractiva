document.addEventListener("DOMContentLoaded", () => {
  ...
  // Define tus cursos por ciclo
  const ciclos = [
    { nombre: "I CICLO", cursos: [
  ...
  ciclos.forEach(ciclo => {
    ...
    cont.className = "cursos-ciclo";
    ciclo.cursos.forEach(c => {
      estado[c.nombre] = false;
      const div = document.createElement("div");
      div.className = "curso";
      div.textContent = c.nombre;
      div.setAttribute("data-nombre", c.nombre);
      div.addEventListener("click", () => {
        estado[c.nombre] = !estado[c.nombre];
        div.classList.toggle("aprobado", estado[c.nombre]);
        if (c.abre) {
          c.abre.forEach(name2 => {
            const t = document.querySelector(`[data-nombre='${name2}']`);
            if (t && estado[c.nombre]) t.classList.add("habilitado");
          });
        }
      });
      cont.appendChild(div);
    });
    divC.appendChild(cont);
    mallaDiv.appendChild(divC);
  });
});
